...
\begin{enumerate}
    ...
    \item \textbf{Propagazione dell'impatto.} L'attaccante esfiltra dati riservati, genera output audiovisivi non autorizzati o contraffà la voce dell'avatar al di fuori del workflow legittimo, estendendo il \emph{blast radius}.
\end{enumerate}
...
L'architettura Dual-Brain trae effettivo beneficio dal disaccoppiamento operativo solo se la barriera di agency viene formalizzata a livello di policy di sicurezza e identità infrastrutturali. Nella baseline manca un vincolo rigoroso per-job sulle chiamate interne; $M11$ e controlli IAM dedicati vengono integrati nella secure architecture proprio per chiudere tali relazioni di fiducia implicita.
...
Lo scenario dimostra che STRIDE-AI e $\text{CIA}^3$-R, pur necessari, richiedono l'integrazione della dimensione \textbf{socio-tecnica}: l'impatto tecnico (violazione di integrità o confidenzialità) viene catalizzato dalla manipolazione della fiducia umana modellata da OWASP ASI09.
