...
\cite{owaspagentic}.
La relazione definisce lo scope, esplicita le assunzioni architetturali, valuta l'applicabilità dei rischi OWASP e approfondisce cinque scenari prioritari: goal hijack del Fast-Brain, poisoning persistente del feedback, abuso di identità e privilegi tra agenti, tampering/replay dei messaggi inter-agent e sfruttamento della fiducia tramite voice cloning.
...
dell'abuso, mentre D10 associa autorizzazione, job e sessione. M1, M2
...
I guardrail devono essere progettati distinguendo i controlli che
devono operare nel percorso real-time da quelli che possono essere
eseguiti fuori dal percorso critico.

\textbf{Controlli sincroni sul Fast-Brain:}
...
\begin{itemize}
    ...
    output non conforme o superamento del timeout.
\end{itemize}

\textbf{Controlli sulla risposta del Deep-Brain:}
...
\begin{itemize}
    ...
    \item blocco di contenuti incompatibili con ruolo e policy del
    personaggio;
...
L'\emph{Human-in-the-Loop} non viene invece inserito in ogni turno
...
corrispondente separazione tecnica:
...
\begin{itemize}
    ...
    credenziali short-lived, evitando token condivisi tra i due agenti;

    \item le policy di rete e filesystem sono \emph{deny-by-default};
    ...
    pipeline di aggiornamento separata;
...
\begin{itemize}
    ...
    \item accessi a risorse incompatibili con l'identità o il ruolo del
    componente;
    \item replay, session mismatch, job mismatch e sequence duplicate;
    ...
    \item transizioni impossibili della state machine, playback
    concorrenti o associazioni cross-sessione;
...
esecuzione anomala prima che il guasto si propaghi.
...
Il red teaming deve comprendere casi multilingue e multimodali,
incluse varianti in arabo e dialetto emiratino, prompt injection su
partial transcript, manipolazione del feedback, replay dei messaggi di
coordinamento e tentativi di abuso della fiducia dell'utente.
...
\begin{itemize}
    \item \textbf{P1 - Least Agency} [Asset $M1$, $D5$]:
    Restringe drasticamente il perimetro d'azione dell'agente Fast-Brain ($M1$), imponendo che possa produrre esclusivamente filler verbali e trigger di animazione ($D5$) appartenenti a una \emph{whitelist} predefinita ed enumerata. Viene negato all'agente rapido qualsiasi accesso diretto a database vettoriali, memoria di sessione o strumenti esterni, limitando il raggio d'impatto di un eventuale \emph{goal hijacking} sull'Instant Path.

    \item \textbf{P2 - Guardrail deterministici e HITL} [Asset $M11$, $D2$, $D3$, $D6$]:
    Introduce nel modulo di guardrail e validazione ($M11$) controlli sincroni e deterministici ad altissima velocità ($<800\,\text{ms}$) sia per il Fast-Brain ($M1$) sia per il Deep-Brain ($M2$). Tali controlli operano sulle trascrizioni parziali e complete ($D2, D3$) imponendo schemi di output rigidi (\emph{JSON Schema strict}), filtri euristico-semantici per rilevare tentativi di prompt injection/imperativi e il blocco immediato di qualsiasi richiesta di dati personali o consigli fuori policy sui testi generati ($D6$). Qualora il guardrail rilevi un'anomalia o un timeout, l'output deviato viene scartato e sostituito da una risposta di \emph{fallback} statica e sicura.
...
\begin{itemize}
    ...
    $M4$]:
    Mantiene l'indice vettoriale FAISS e la memoria di contesto ($D4$) accessibili in scrittura
    esclusivamente tramite una pipeline di ingestione separata e controllata, isolata dal percorso
    conversazionale in tempo reale; il componente di retrieval ($M4$) opera in lettura sola durante
    l'inferenza, impedendo che l'interazione utente possa in alcun modo alterare direttamente
    l'indice.
...
Un attaccante che abbia precedentemente deviato il Fast-Brain, ad
esempio tramite lo Scenario~1, non ottiene direttamente la capacità di
invocare i servizi multimediali. Tuttavia può tentare di sfruttare la
relazione di fiducia con M9 inducendo quest'ultimo ad eseguire una
richiesta formalmente valida utilizzando privilegi più elevati.
...
\begin{enumerate}
    ...
    Esiste almeno una debolezza nel modello IAM della pipeline:
    credenziali condivise, token con scope superiore al necessario,
    identità non separate tra workload, assenza di audience restriction
    oppure insufficiente binding fra identità, sessione e job.
