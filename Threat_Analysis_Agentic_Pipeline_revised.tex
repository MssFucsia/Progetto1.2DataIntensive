...
inerente}, ossia il rischio precedente all'applicazione delle
mitigazioni proposte nelle sezioni successive.

Nella valutazione vengono considerate esclusivamente le caratteristiche
dell'architettura definita nello scope corrente, tra cui la separazione
funzionale tra Fast-Brain e Deep-Brain, il parallelismo dei due
percorsi, la presenza di memoria e feedback persistenti, la sequenza
deterministica dei servizi multimediali e l'assenza di esecuzione
dinamica di codice.
...
&
\textbf{L: Bassa.}
Lo sfruttamento richiede il bypass dei controlli o la compromissione
dell'Orchestratore, poiché i servizi sono invocati secondo una sequenza
predefinita.
\newline \textbf{S: Alta.}
...
\begin{enumerate}
    ...
    direttiva prevalente ed effettuando l'output, questo formalmente rispetta lo schema JSON, la
    allowlist dei trigger e i vincoli di lunghezza previsti; tuttavia il filler verbale e/o il
    trigger comportamentale selezionati risultano semanticamente inappropriati rispetto al ruolo e
    al tono del personaggio.
    ...
    \item \textbf{Bypass del guardrail.} Il modulo di guardrail e validazione (M11) opera controlli
    sintattici e strutturali -- conformità allo schema, validità del \texttt{session\_id},
    monotonicità della sequenza, appartenenza del trigger a un'allowlist, lunghezza del filler -- ma
    non dispone di un livello di validazione semantica in grado di rilevare un contenuto
    strutturalmente legittimo ma tonalmente o eticamente inappropriato. L'output, pur deviato
    dall'hijack, supera quindi la validazione ed è inoltrato ai motori downstream di sintesi vocale
    e rendering grafico.
\end{enumerate}
...
Il threat si concretizza sfruttando la natura stocastica dei modelli generativi, la stretta finestra
temporale di elaborazione dell'Instant Path ($<$800\,ms) -- che limita la capacità di applicare
controlli di sicurezza complessi prima della generazione dell'output -- e il divario tra validazione
sintattica e validazione semantica del guardrail (M11), che consente il transito di output
formalmente conformi ma comportamentalmente deviati.
...
\begin{enumerate}
    ...
    \item \textbf{Aggiornamento del ranking/memoria.} In assenza di validazione robusta della
    provenienza e di soglie anti-abuso, il componente di retrieval (M4) e/o il Feedback Manager
    aggiornano lo score di similarità, il ranking dei documenti o il pool di few-shot examples,
    promuovendo stabilmente il contenuto avvelenato all'interno della Knowledge Base e della memoria
    contestuale (D4).
    ...
    \item \textbf{Generazione dell'output dannoso.} Il Deep-Brain genera una risposta fondata sul
    contenuto avvelenato (ad esempio un dosaggio erboristico non sicuro), che il guardrail (M11) --
    concentrato su conformità di schema e allowlist dei trigger, non su una verifica di merito del
    contenuto informativo recuperato dal RAG -- non è strutturato per intercettare, poiché l'output
    resta testualmente coerente, ben formato e in linea con il tono del personaggio.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Validazione incompleta.}
    M11 verifica che il messaggio rispetti lo schema previsto, ma
    l'assenza di un controllo sufficiente su origine, freshness,
    unicità e binding con la sessione consente al payload formalmente
    valido di essere accettato.
...
\begin{enumerate}
    ...
    \item \textbf{Validazione insufficiente del merito.}
    M11 accetta un output grammaticalmente corretto e coerente con la
    personalità, ma non identifica la richiesta indebita di
    informazioni personali o la natura non sicura
    dell'affermazione proposta.
...
La secure architecture proposta mantiene invariata la struttura
funzionale Dual-Brain descritta nella
